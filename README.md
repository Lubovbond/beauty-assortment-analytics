...
* Реализован сбор и трансформация данных из двух изолированных источников:
  * *Транзакционная таблица по товарам:* текущие розничные цены, фактическая выручка, маржинальность и оборачиваемость.
  * *Историческая выгрузка финансовых метрик:* агрегированные показатели брендов в динамике по месяцам.
...
* **[notebooks/](notebooks/)** — интерактивный Jupyter-ноутбук `beauty_analytics_demo.ipynb` с пошаговой генерацией синтетических данных бьюти-ритейла, расчетом операционной выручки, динамики MoM и выявлением брендов в зоне риска.
* **[sql/](sql/)** — готовые SQL-скрипты `kpi_calculations.sql` для СУБД (PostgreSQL/ClickHouse) с расчетом процента выкупа (buyout rate), объемов продаж и фильтрацией аутсайдеров матрицы через операторы `CASE WHEN` и `HAVING`.
